...
var msgs = {
    ...
    ERR_TAGS_INVALID: 'The "tags" field must be an array of strings',
    ERR_TAG_ITEM_INVALID: 'Invalid tag item encountered: '
};
...
function testTags(md, p) {
    ...
    if (!md.tags) {
        ...
        if (Array.isArray(md.tags)) {
            md.tags.forEach(function (curTag) {
                if (typeof curTag !== "string") {
                    errors.push(
                        msgs.ERR_TAG_ITEM_INVALID +
                            "'" + curTag + "'"
                    );
                }
            });
        } else {
            errors.push(msgs.ERR_TAGS_INVALID);
        }
    }
