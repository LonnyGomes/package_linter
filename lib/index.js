    ...
    fs.stat(pathName, function (err, stats) {
        ...
        if (err) {
            ...
            if (stats.isDirectory()) {
                ...
                //since it's a filename, make sure it ends in .visual
                if (!extName.match(/^\.visual$/)) {
                    deferred.reject('invalid extension! File must end in .visual');
                } else {
                    promise = processPackage(pathName);
                }
            }

            //continue if we've either processed path or package
            if (promise) {
